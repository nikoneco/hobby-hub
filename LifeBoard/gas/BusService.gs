...
function fetchRouteSnapshotSafely_(route) {
  try {
    ...
    if (stored) {
      stored.errorText = 'GAS direct fetch failed; using local snapshot. ' + (error && error.message ? error.message : String(error));
      return stored;
    }
    return {
...
function getStoredBusRouteSnapshot_(route) {
  try {
    ...
    snapshot.sourceUpdatedAtText = snapshot.sourceUpdatedAtText
      ? snapshot.sourceUpdatedAtText + ' / PC同期'
      : 'PC同期';
    snapshot.importedAt = importedAt;
    snapshot.source = String(row.source || 'local-sync');
    return snapshot;
  } catch (error) {
